...
AND ((a.start_time<=b.end_time AND a.start_time>=b.start_time) OR
(a.end_time<=b.end_time AND a.end_time>=b.start_time));
...
SELECT distinct student.stu_id,stu_name
FROM student
inner join takes on student.stu_id=takes.stu_id
WHERE EXISTS(
	SELECT 1
	FROM takes t1
...
AND (
	...
	FROM takes t3 
	WHERE t3.stu_id=student.stu_id
)>(
...
SELECT distinct student.stu_name,cor_name
FROM student 
join takes on student.stu_id=takes.stu_id
join course on course.cor_id=takes.cor_id
...
(
	...
	WHERE s3.stu_name=student.stu_name
	AND t1.cor_id=takes.cor_id
)